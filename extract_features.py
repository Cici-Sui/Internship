import os   #导入各种数据库  import os 是用来导入 os（operating system）模块 的语句，该模块提供了与操作系统交互的功能，比如文件和目录管理、进程管理、环境变量访问等。

import pandas as pd
import numpy as np 
import numpy.matlib as matlib    #pandas numpy这些用于数据处理
import scipy
import scipy.signal
import scipy.stats
import scipy.io.wavfile
import scipy.fftpack    # scipy用于信号处理（滤波、变换等）

from pynwb import NWBHDF5IO   # 用于处理 Neurodata Without Borders (NWB) HDF5 文件。
import MelFilterBank as mel

#Small helper function to speed up the hilbert transform by extending the length of data to the next power of 2
hilbert3 = lambda x: scipy.signal.hilbert(x, scipy.fftpack.next_fast_len(len(x)),axis=0)[:len(x)] # 计算 Hilbert 变换，用于获取信号的包络信息， 括号里的第二个parameter选择一个更快的 FFT 计算长度，加速运算

def extractHG(data, sr, windowLength=0.05, frameshift=0.01):   #  提取高伽马（High-Gamma, HG）特征
    """
...
def stackFeatures(features, modelOrder=4, stepSize=5):  # 特征堆叠（时间上下文）
    """
...
def downsampleLabels(labels, sr, windowLength=0.05, frameshift=0.01):  # 标签降采样
    """
    ...
    for w in range(numWindows):
        ...
        stop = int(np.floor(start+windowLength*sr))
        newLabels[w]=np.unique(labels[start:stop])[0].encode("ascii", errors="ignore").decode()  #这里也有问题 硕士第一个‘a'那里不是numeric
    return newLabels

def extractMelSpecs(audio, sr, windowLength=0.05, frameshift=0.01):  # 提取 Mel 频谱特征
    """
    ...
    win = scipy.signal.windows.hann(int(windowLength*sr + 1))[:-1]
...
def nameVector(elecs, modelOrder=4): # 特征名称生成
    """
...
if __name__=="__main__": # 主流程
    ...
    modelOrder = 4
    stepSize = 5
    path_bids = r'./SingleWordProductionDutch-iBIDS' # 数据储存的路径
    path_output = r'./features' # 特征输出路径
    ...
    for p_id, participant in enumerate(participants['participant_id']):
        ...
        eeg_sr = 1024  #设定 eeg采样率
        #audio
        audio = nwbfile.acquisition['Audio'].data[:]
        audio_sr = 48000  #  # 设定 sudio 采样率 sampling rate?
        #words (markers)
        words = nwbfile.acquisition['Stimulus'].data[:]  # 获取标签数据
        words = np.array(words, dtype=str) # 转换为字符串数组
        io.close()
        #channels
        ...
        #Stack features
        # feat = stackFeatures(feat,modelOrder=modelOrder,stepSize=stepSize)  把这行comment掉了
        
        #Process Audio
        target_SR = 16000
        audio = scipy.signal.decimate(audio,int(audio_sr / target_SR))
        audio_sr = target_SR # 降采样后，更新了音频的采样率
        ...
        # melSpec = melSpec[modelOrder*stepSize:melSpec.shape[0]-modelOrder*stepSize,:]  213和214也comment掉了
        ...
        #Save everything
        np.save(os.path.join(path_output,f'{participant}_feat.npy'), feat) # (30025, 127) feat: array (windows, channels) Frequency-band feature matrix
        np.save(os.path.join(path_output,f'{participant}_procWords.npy'), words) # (30025,) newLabels: array of str ， Downsampled labels
        np.save(os.path.join(path_output,f'{participant}_spec.npy'), melSpec)  # (30025, 23), spectrogram: array (numWindows, numFilter)
        np.save(os.path.join(path_output,f'{participant}_feat_names.npy'), feature_names) # (1143,) names: array of str
        # --List of electrodes including contexts, will have size elecs.shape[0]*(2*modelOrder+1)
        # 1143 = 127*9 = 127*(2*4+1)
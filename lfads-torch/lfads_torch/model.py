...
class LFADS(pl.LightningModule):
    ...
    def _shared_step(
        ...
        # # Get prediction and ground truth as numpy [32, 100, 23]
        # out_means = [self.recon[s].compute_means(output[s].output_params) # output_params  torch.Size([32, 100, 23, 1])
        #              for s in sessions]  
        # # Compute Pearson r for each frequency bin across all time/windows:
     
        # spectrogram = batch[0].recon_data.cpu().numpy() # just use [0] for now since running single session
        # rec_spec = out_means[0].detach().cpu().numpy()
        
        # n_windows = spectrogram.shape[0]
        # F_bins = spectrogram.shape[2]   
        # rs = np.zeros((n_windows,F_bins))
        # for k in range(n_windows):
        #     for specBin in range(F_bins):
        #         if np.any(np.isnan(rec_spec[k,:,specBin])):
        #             print(f"Window{k}, bin {specBin} has {np.sum(np.isnan(rec_spec[k,:,specBin]))} NaNs")
        #         r, p = pearsonr(spectrogram[k,:,specBin], rec_spec[k,:,specBin])
        #         rs[k,specBin] = r
        # mean_r = np.mean(rs)
        
        
        # self.log(f"{split}/pearson_r", mean_r, on_step=False, on_epoch=True, prog_bar=True)
        # ———————————————————
...
#                      for s in sessions]  
#         # Compute Pearson r for each frequency bin across all time/windows:
#         # print("here here here here here",len(out_means[0][0][0])) #debug
#         spectrogram = [batch[s].recon_data.cpu().numpy() for s in sessions]
#         rec_spec = [out_means[s].detach().cpu().numpy() for s in sessions]
        
#         n_windows = spectrogram.shape[0]
#         F_bins = spectrogram.shape[2]   
#         rs = np.zeros(F_bins)
#         for specBin in range(F_bins):
#             if np.any(np.isnan(rec_spec[:,:,specBin])):
#                 print(f"Bin {specBin} has {np.sum(np.isnan(rec_spec[:,:,specBin]))} NaNs")
...
#         # ———————————————————
    ...
    def validation_step(
        ...
        # 【DEBUG】print(f"[DEBUG] validation_step: model.training = {self.training}")
        """
    ...
    def on_validation_epoch_end(self):
        ...
        #【DEBUG】print(f"[DEBUG] Ending validation: model.training = {self.training}")
        # run epoch‐wide Pearson’s r on TRAIN and VALIDATION
        self._evaluate_epoch(split="train")
        self._evaluate_epoch(split="valid")
        
    def _evaluate_epoch(self, split: str):
        from scipy.stats import pearsonr
        ...
        # get data through each training mini-batch
        all_pred, all_true = [], []
        for batch in loader:
            sb, _ = batch[0] # session batch and info with it
            out = self.forward({0: sb},
                               sample_posteriors=False,
                               output_means=True)
            pred = out[0].output_params.squeeze(-1).detach().cpu().numpy()
            true = sb.recon_data.cpu().numpy()
            all_pred.append(pred) 
            all_true.append(true)


        # collect 3D data for whole epoch
        recon_w = np.concatenate(all_pred, axis=0) # e.g. sub-01 recon_w.shape-> (2368,100,23)
        truth_w = np.concatenate(all_true, axis=0)
        n_w, T, F = recon_w.shape
        shift = 10

        # stitch windows back to 2D without overlaping
        full_pred  = self._stitch_windows(recon_w, shift) # e.g. sub-01 (23770.23)
        full_truth = self._stitch_windows(truth_w, shift)

        # per‐frequency bin Pearson’s r
        rs = np.zeros(F)
        for f in range(F):
            rs[f], _ = pearsonr(full_truth[:, f], full_pred[:, f])
        mean_r = float(rs.mean())

        # print & log
        print(f"[Epoch {self.current_epoch:3d}] {split}/pearson_r_epoch = {mean_r:.4f}")
        self.log(f"{split}/pearson_r_epoch", mean_r,
                 on_step=False, on_epoch=True, prog_bar=True)   
    
    def _stitch_windows(self, data: np.ndarray, shift: int) -> np.ndarray:
        # stitch data back to 2D (take out the overlaps)   
        n_w, T, F = data.shape  
        overlap    = T - shift
        L          = T + (n_w - 1) * shift
        full = np.zeros((L, F), dtype=data.dtype)

        # copy the entire first window
        full[:T] = data[0] 
        # append only each window’s new 'shift' tail
